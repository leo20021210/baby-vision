...
def load_split_train_test(datadir, args):
    ...
    train_data = datasets.ImageFolder(osp.join(datadir, 'train'), transform=transforms.Compose([transforms.ToTensor(), normalize]))
    test_data = datasets.ImageFolder(osp.join(datadir, 'val'), transform=transforms.Compose([transforms.ToTensor(), normalize]))

    print('Training data size is', len(train_data))
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    optimizer = torch.optim.Adam(model.module.fc.parameters(), args.lr, weight_decay=args.weight_decay)
    cudnn.benchmark = True

    # Data loading code
    savefile_name = args.model_name + '_toybox_50_iid.tar'
